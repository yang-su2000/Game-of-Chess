...
using namespace std;
ComputerLevelThree::ComputerLevelThree(Colour colour, ChessBoard &b):
	Player{colour,0,"computer[3]",false}, b{b}{}
void ComputerLevelThree::toggleDebug(){
	debug = !debug;
}
void ComputerLevelThree::printMoves(){
...
void ComputerLevelThree::checkAvoid(int r, int c, Info selectedPiece){
...
void ComputerLevelThree::checkCheck(int r, int c, Info enemyKing){
...
void ComputerLevelThree::checkCapture(int r,int c, Info selectedPiece){
...
Move ComputerLevelThree::nextMove(){
	...
	if(!isInvalid){
		...
		if(debug) printMoves();
		//I have no more moves to make, so I am either in checkmate or stalemate
		if(moveList.empty()){
